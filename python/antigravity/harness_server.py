    ...
    async def Connect(self, request_iterator, context):
        ...
        async for request in request_iterator:
            if request.WhichOneof("type") != "start":
                continue  # cancel request not handled yet
    ...
    async def _run_turn(self, request):
        ...
        if latest_message.content.WhichOneof("type") != "text":
            yield ax_pb2.HarnessResponse(
                conversation_id=request.conversation_id,
                    ...
                    state=ax_pb2.STATE_FAILED,
                    error_message="Latest message must contain text content",
                ),
            )
            return
        ...
        if not loaded_config:
            yield ax_pb2.HarnessResponse(
                conversation_id=request.conversation_id,
                    ...
                    state=ax_pb2.STATE_FAILED,
                    error_message="Agent config is not loaded on the server",
                ),
            )
            return
        ...
        try:
            async with Agent(loaded_config) as agent:
                ...
                # Hydrate history
                print(
                    f"[gRPC] Hydrating {len(historical_messages)} historical messages..."
                )
                    ...
                    if isinstance(chunk, Text):
                        msg = ax_pb2.Message(
                            ...
                            content=content_pb2.Content(
                                text=content_pb2.TextContent(text=chunk.text)
                            ),
                        )
                        yield ax_pb2.HarnessResponse(
                            conversation_id=request.conversation_id,
            ...
            yield ax_pb2.HarnessResponse(
                conversation_id=request.conversation_id,
                    ...
                    state=ax_pb2.STATE_FAILED,
                    error_message=f"Agent execution terminated due to error. ({str(e)})",
                ),
            )
            return
...
async def serve(host: str, port: int):
    ...
    ax_pb2_grpc.add_HarnessServiceServicer_to_server(
        AntigravityHarnessServiceServicer(), server
    )
...
def main():
    ...
    parser.add_argument(
        "--agent_file",
        default="examples/antigravity_agent/agent.py",
